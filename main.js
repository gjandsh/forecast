                ...
                $.get('https://weixin.jirengu.com/weather/now',
                    { cityid: cid }).done(function (res) {
                        ...
                        for (i = 0; i < 7; i++) {
                            ...
                            $('.today').eq(i).children('.temperature').text(avetem + '°');
                            let futemrange = res.weather[0].future[i].low + "°~" + res.weather[0].future[i].high;
                            $('.not-today').eq(i).children('.temperature').text(futemrange + "°");
                            let nowtem = res.weather[0].now.temperature
                            $('.today').eq(0).children('.temperature').text(nowtem + '°');
